...
8. End, render, and open the report: `npx dailies-cli session end "$id" --open`. The `--open`
   ...
   yourself.) Then report the `report.html` and video paths. One video is recorded per page and only one
   gets finished — the page your last step ended on. Pass `--video <page name>` when that isn't
   the page you were demonstrating.
